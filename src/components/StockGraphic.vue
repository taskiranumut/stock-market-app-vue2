...
export default {
  name: "StockGraphic",
  computed: {
    ...mapState(["currentSymbolData"]),
    formattedSymbolData() {
      let datas = this.currentSymbolData.map((item) => {
        return {
...
High: ${formatValue(Yh[i])}`;
    ...
    makeChart() {
      this.CandlestickChart(this.formattedSymbolData, {
        date: (d) => new Date(d.Date),
        high: (d) => d.High,
        low: (d) => d.Low,
        open: (d) => d.Open,
        close: (d) => d.Close,
        xDomain: this.currentSymbolData
          .map((item) => new Date(Object.keys(item)))
          .reverse(),
        xTicks: this.currentSymbolData
          .map((item) => new Date(Object.keys(item)))
          .filter((item, index) => {
